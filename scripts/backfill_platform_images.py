...
def main(dry_run: bool = False):
    from backend.content.web_images import inject_platform_images, platforms_in
    ...
    for row in rows:
        ...
        if "/assets/platforms/" in body:
            skipped += 1
            continue

        language = row.get("language", "en")
        keyword = row.get("primary_keyword", "")
        new_body = inject_platform_images(body, keyword, language=language)

        if new_body == body:
            skipped += 1
            continue

        names = [p["name"] for p in platforms_in(keyword + " " + body)][:4]
        n_figs = new_body.count("/assets/platforms/")
        print(f"→ {row['region']}:{row['pillar']}:{row['slug']} ({language}) — {n_figs} logo('s): {', '.join(names)}")
    ...
    print(f"\n{updated} pagina('s) {verb}, {skipped} overgeslagen (al beelden / geen platform genoemd)")
